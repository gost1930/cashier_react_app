        ...
        <thead className="border-b font-medium dark:border-neutral-500  border">
          <tr>
            {Object.values(columns).map((column, index) => (
              <th key={index} scope="col" className="border bg-white p-2">
                {column}
              </th>
            ))}
            <th scope="col" className="py-2 border bg-white">
              ------
            </th>
          </tr>
        </thead>
          ...
          {rows?.map((row, index1) => (
            ...
            >
              {Object.keys(columns).map((cell, index2) => (
                <td
                  key={index2}
                  className="whitespace-nowrap max-w-56 overflow-auto scrollbar-thin font-medium bg-white"
                > 
                  {row[cell]}
                </td>
              ))}
              <td className="whitespace-nowrap px-6 gap-x-8 flex relative bg-white">
                {!readOnly && (
                  <>
                    {" "}
                    <CiEdit
