import { LOGIN_URL, LOGOUT_URL, USERS_URL, ADD_USERS_URL, DELETE_USERS_URL, UPDATE_USERS_URL } from "../consts";

export const getUsers = async (id, page) => {
  const API_ENDPOINT = id ? `${USERS_URL}/${id}/${page}` : `${USERS_URL}/${page}`
  const headers = page === "ADD_USER" ? {
...
export const logout = async() => {
  ...
  if (response.status === 200) {
    ...
    throw new Error("غير مسموح لك تسجيل الخروج");
  } else {
    throw new Error("حدث خطأ ما ، الرجاء المحاولة مرة اخرى");
  }
};
...
export const addUser = async (data) => {
  ...
  if (response.status === 201) {
    ...
    throw new Error("غير مسموح لك  اضافة  المستخدم");
  }
  else {
    throw new Error("حدث خطأ ما ، الرجاء المحاولة مرة اخرى");
  }
...
export const updateUser = async (id, data) => {
  ...
  if (response.status === 200) {
    ...
    throw new Error("غير مسموح لك تعديل بيانات المستخدم");
  }
...
export const deleteUser = async (id) => {
  ...
  if (response.status === 200) {
    ...
    throw new Error("غير مسموح لك حذف المستخدم");
  } else {
    throw new Error("حدث خطأ ما ، الرجاء المحاولة مرة اخرى");
  }
};
